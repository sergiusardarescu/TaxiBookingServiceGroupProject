// The imports used for the window.
...
public class LoggedCust extends JFrame  implements ActionListener
{
	/**
	 * 
	 */
	...
	private String lastOrderDriver;
	private JLabel lastOrder;
	private JTextArea lastOrderInfo;
	private LoggedCust thisPage;
	...
	public LoggedCust(Customer app, String userId)
	{
	...
	// issue when interacting with it.
	private void setupCustomer() {
		...
		// even if it does not fit in the pane.
		// The setViewportView method basically turns the scrollPane into a
		// container that can accomodate the table. In other words, the scrollPane already has a
		// single component attached to it, so setViewportView can actually add an element.
		...
		// This represents the creation of the textArea defined previously.
		// It creates a link to the method used for the display of the details.
		...
		bookingBtn.setBounds(744, 700, 136, 40);
		bookingBtn.addActionListener(this);
		bookingBtn.setActionCommand("booking");
	...
	public void retrieveOrders() {
		...
		try {
			connection = DriverManager.getConnection("jdbc:mysql://localhost/group28","root","");
			pst = connection.prepareStatement("select * from booking where user_id = " + this.user);
			rs = pst.executeQuery();
	...
	public void retrieveLastOrder() {
		...
		try {
			connection = DriverManager.getConnection("jdbc:mysql://localhost/group28","root","");
			pst = connection.prepareStatement("select * from booking where (status = 'pending' OR status = 'taken') AND user_id = " + this.user + " ORDER BY created_at DESC" );
			rs = pst.executeQuery();
			if (rs.next()) {
				lastOrderId = rs.getString(1);
				lastOrderDriver = rs.getString(9);
	...
	// Both of the statements are joined in a batch and executed.
	public void cancelLastOrder() {
		Connection connection;
		try {
			connection = DriverManager.getConnection("jdbc:mysql://localhost/group28","root","");
			Statement stmt = connection.createStatement();
			String sql1 = "UPDATE drivers SET available = 0 WHERE id = " + this.lastOrderDriver;
			String sql2 = "UPDATE booking SET status = 'canceled' where id = " + this.lastOrderId;
			stmt.addBatch(sql1);
			stmt.addBatch(sql2);
			stmt.executeBatch();
	...
	// on each button.
	// If the action equals to "booking" it makes the window invisible and
	// opens a new booking app that remembers the user in this window.
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(action.equals("booking")) {
			setVisible(false);
			new Booking(thisPage, this.user);
